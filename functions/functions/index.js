...
exports.requestCreated = functions.firestore.document("/requests/{id}")
    .onCreate(async (snap) => {
...
exports.playerCreated = functions.firestore.document("/challenges/{id}/players/{playerId}")
...
exports.playerRemoved = functions.firestore.document("/challenges/{id}/players/{playerId}")
    .onDelete(async (snap, context) => {
        const challengeId = context.params.id;
        const playersSnap = await admin.firestore().collection(`challenges/${challengeId}/players`).get()
        if (playersSnap.size == 0) {
            return admin.firestore().collection("challenges").doc(challengeId).delete()
        }
        return true;
    });
    ...
    async (change, context) => {
        ...
        userStatusFirestoreRef.set({ status }, { merge: true }).then(() => {
            if (status === "offline") {
                ...
                admin.firestore()
                    .collectionGroup("players").where("id", "==", uid).get().then((snapshot) => {
                ...
                admin.firestore()
                    .collection("requests").where("uid", "==", uid).get().then((snapshot) => {
                        snapshot.docs.forEach((doc) => {
...
const updateChallengeStatus = async (challengeId) => {
    const playersSnap = await admin.firestore().collection(`challenges/${challengeId}/players`).get()
    const challengeQuery = admin.firestore().collection("challenges").doc(challengeId)
    const challenge = await challengeQuery.get()
    const players = challenge.data().players
    admin.firestore().collection("challenges").doc(challengeId)
        .set({
